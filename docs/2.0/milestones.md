...
| --- | --- |
| 分支 | `refactor/2.0.0` |
| 当前里程碑 | **M1 🟡 代码完成**（待验证码窗口过期后实测） |
| 上次更新 | 2026-09-13 23:00 |
| 阻塞项 | 登录触发图形验证码（已等待 60 秒仍触发） |

**下一步动作**：
1. 等待更长时间（验证码窗口通常 5-30 分钟）或使用浏览器手动登录一次清除验证码状态
2. 重试 `deno task keepalive` 完成 M1 门禁验证
3. 所有代码已就绪并提交（commit fc1c165），56 个单测全过

**M1 已完成项**：
- ✅ 全部协议实现（Clink 四步握手、三通道编排、就绪掩码判定）
- ✅ 16 个 Clink 单测（frame/reassembler/ticket/channel）
- ✅ `keepalive` CLI 工具
- ✅ 内置 WebSocket 验证（支持 `protocols` 数组 + `headers`）
...
| M5 | 桌面外壳 | ⬜ 未开始 | Windows 托盘常驻 + toast 通知 |
...
- [ ] `deno task css`：Tailwind 4 + daisyUI 5 构建链路（**不需要 Node**）
- [ ] `deno task client`：`deno bundle` 产出 `assets/client.js`（`hono/jsx/dom`，实测 15.2 KB minified）
- [ ] `server/app.tsx` + `ssr.tsx`：Hono 路由 + JSX 首屏外壳
- [ ] `server/stream.ts`：SSE `snapshot` / `patch` / `log` 三类事件，带单调 `rev`
- [ ] `server/api.ts`：全部 POST 命令接口 + 重复入队去重
- [ ] 客户端 store：EventSource 接入 + 断线自动重连 + `rev` 不连续时重拉全量
- [ ] **账号 tab 条**：状态点、删除 ×、尾部 ＋、GitHub 按钮、横向滚动、双击重命名、零账号空状态
  - [ ] 新增账号对话框：必填内联校验（`aria-invalid` + 红字 + 移焦）/ 服务端错误走 Toast
  - [ ] 删除两步确认，正文列出恰好三条后果
  - [ ] tab 名：无别名显示完整账号，有别名只显示别名；hover 两行原生 title；**必须用 `textContent` 写入**
- [ ] **保活 tab**：间隔输入（1–59，失焦钳制 + 1 秒高亮，无保存按钮）、设备列表、mm:ss 倒计时（客户端本地算）、`desktopCode` 点击复制、手动按钮 5 态、关机灰显、`needLineUp` 徽章、`forbiddenConnect` 灰显
  - [ ] 刷新：手动 10s 节流 / 进 tab >60s 刷新 / 后台→前台且有焦点且 >60s；三者共用同一条代码路径
  - [ ] 失败保留旧列表 + 陈旧时间戳横幅 + 重试按钮；首次加载 3 行骨架
- [ ] **日志 tab**：时间/等级/模块/对象/消息五列、跨天分隔、自动滚动 + 上滚关闭 + 「回到底部」、当前账号范围 + 系统日志开关
- [ ] 登录失败红色横幅 + 需人工处理琥珀色横幅 + 「重新登录」（回显密码的唯一入口）
- [ ] 可访问性硬要求：hover 只改背景与边框不改文字对比度、焦点环、最小点击区 28×28、颜色不作唯一信息载体
- [ ] 无头模式：控制台打印地址；`--host` 非 loopback 时强制要求 `--token`

**门禁**
- [ ] 浏览器打开 127.0.0.1，能完成增删账号、开关自动保活、手动保活、改间隔、看日志全流程
- [ ] 倒计时每秒走字且不打断滚动位置 / 输入焦点 / 「已复制」提示
- [ ] 开关打开后 1 秒内开始执行；失败后开关保持打开
- [ ] 拔网线 → 顶部横幅 → 恢复后无请求风暴
...
- [ ] 任务 1003（使用 1 小时）：复用保活 ①②③ 并保持三通道
  - [ ] `SET_ACK(type=3)` → `ACK_SYNC(type=1)` **必答**，并持续排空消息
  - [ ] 先读进度只补剩余：`ceil(remaining/60) × 60 + 60`
  - [ ] 每 5 分钟重读进度，接口值永远覆盖本地
  - [ ] 被踢（会话冲突）**不重连不回收**；网络掉线重连 3 次（5/15/45s），每次重新竞速连接信息并重读进度
  - [ ] 与周期保活互斥；取第一台 `useStatus="25"` 的设备，无则当日跳过并记 INFO
- [ ] 时间窗：默认 09:00–11:30、每日随机一个开始时间、跨天提示、<10 分钟回退、窗口只约束开始、错过跳过不补、手动执行忽略窗口
- [ ] **积分 tab**：余额大号等宽千分位、任务列表六列（任务/奖励/进度/状态/执行/自动）、进度单位推断（1→次，3600→分钟，其他→裸比例）、状态胶囊、未实现 `eventType` 灰显 + 悬浮说明
- [ ] AI 侧受阻只挂起 1004，不改账号状态点；连续两次 AI 密码错误触发保护

**门禁**
- [ ] 一个真账号在一个时间窗内三个任务全部 `status=2`
- [ ] 1003 跑满 1 小时，每 5 分钟进度被接口值纠正，`ACK_SYNC` 正常应答
- [ ] 在 35 分钟处杀进程，重启后只补剩余时长，累计进度未清零
- [ ] 1004 全程未向用户索要密码
- [ ] 未知 `eventType` 任务不破坏页面
